...
def get_facade_list(url, db_local_node, get_facade_from_url):
    facade_list = []
    local_engine_facade = get_facade_from_url(
        replace_node_ip(url, db_local_node))
    facade_list.append(local_engine_facade)
    nodes = retrieve_nodes(local_engine_facade.get_engine())
    # Remove local node since we already got the connection for it
    if db_local_node in nodes:
        nodes.remove(db_local_node)

    for node in nodes:
        conn_string = replace_node_ip(url, node)
        facade_list.append(get_facade_from_url(conn_string))
    return facade_list


def replace_node_ip(url, node):
    parsed = urlparse.urlparse(url)
    if parsed.netloc:
        return parsed._replace(
            netloc='%(user)s:%(pwd)s@%(node)s' %
                   dict(user=parsed.username,
                        pwd=parsed.password,
                        node=node)).geturl()
    else:
        return url
...
def retrieve_nodes(engine):
    try:
        result = engine.execute(("SHOW VARIABLES LIKE"
                                 " 'wsrep_cluster_address'")).fetchone()
        if result:
            return re.findall(r'[0-9]+(?:\.[0-9]+){3}', result[1])
    except:
        pass
...
def choose_facade(facade_list):
    # Always prefer local, unless it is unhealthy
    def is_healthy(facade):
        # TODO(HEALTHCHECK): light weight health check, how?
        return True

    for facade in facade_list:
        if is_healthy(facade):
            return facade
