...
OPTS = [
    cfg.IntOpt('max_sample_limit',
               default=20,
               help='Upper limit of the number of meter samples to fetch'),
    cfg.BoolOpt('do_app_load_balance',
                default=False,
                help='App does the load balancing for database'),
    cfg.StrOpt('db_local_node',
               default="",
               help='Local database node for retrieving cluster nodes'),
]

cfg.CONF.register_opts(OPTS, group="sqlalchemy")
...
class Connection(base.Connection):
    ...
    def __init__(self, url):
        self._engine_facade_list = []
        if (cfg.CONF.sqlalchemy.do_app_load_balance and
                cfg.CONF.sqlalchemy.db_local_node):
            self._engine_facade_list = sql_utils.get_facade_list(
                url,
                cfg.CONF.sqlalchemy.db_local_node,
                self.get_facade_from_url)
        else:
            self._engine_facade_list.append(self.get_facade_from_url(url))
    ...
    @property
    def _engine_facade(self):
        facade = sql_utils.choose_facade(self._engine_facade_list)
        LOG.info(facade.get_engine())
        return facade
    ...
    def get_meters(self, user=None, project=None, resource=None, source=None,
        ...
        if project is None:
            query_sample = (query_sample
                            .limit(cfg.CONF.sqlalchemy.max_sample_limit))
